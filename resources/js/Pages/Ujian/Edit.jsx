...
export default function Edit({ auth, ujian, jadwals }) {
    const formatDateTimeForInput = (datetime) => {
        if (!datetime) return '';
        try {
            // Karena Laravel mengirim format standar, new Date() akan bekerja dengan benar
            const date = new Date(datetime);
            if (isNaN(date.getTime())) return '';
            
            // Dapatkan komponen waktu lokal dari browser
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        jadwal_pelajaran_id: ujian.jadwal_pelajaran_id || '',
        waktu_mulai: formatDateTimeForInput(ujian.waktu_mulai),
        waktu_selesai: formatDateTimeForInput(ujian.waktu_selesai),
        tipe: ujian.tipe || 'Ujian',
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('ujian.update', ujian.id));
    };


    return (
        <AuthenticatedLayout header="Edit Ujian">
                ...
                <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <form onSubmit={submit} className="p-6 space-y-6">
                        ...
                        <div className="flex items-center justify-end">
                            <PrimaryButton disabled={processing}>Update Ujian</PrimaryButton>
                        </div>
                    </form>
                </div>
            </div>
