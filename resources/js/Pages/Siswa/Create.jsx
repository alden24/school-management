...
export default function Create({ auth, kelas }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        // Data Akun Login (tanpa password)
        email: '',
    ...
    const submit = (e) => {
        e.preventDefault();
        // Gunakan 'post' karena Inertia akan otomatis menangani multipart/form-data
        post(route('siswa.store'));
    };

    return (
        <AuthenticatedLayout
            header="Tambah Data Siswa Baru"
        >
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <form onSubmit={submit} className="p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                                <div>
                                    <InputLabel htmlFor="foto" value="Unggah Foto (Opsional)" />
                                    <input type="file" id="foto" onChange={handleFotoChange} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100" />
                                    <InputError message={errors.foto} className="mt-2" />
                                </div>
                                {fotoPreview && (
                                    <div className="mt-4">
                                        <img src={fotoPreview} alt="Preview Foto" className="w-32 h-32 rounded-full object-cover" />
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="jenis_kelamin" value={data.jenis_kelamin} onChange={(e) => setData('jenis_kelamin', e.target.value)} className="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                <div>
                                    ...
                                    <select id="class_id" value={data.class_id} onChange={(e) => setData('class_id', e.target.value)} className="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            ...
                            <p className="text-sm text-gray-500 mb-4">Password akan dibuat secara otomatis berdasarkan tanggal lahir siswa (format: ddmmyyyy) atau 8 karakter acak jika tanggal lahir kosong.</p>
