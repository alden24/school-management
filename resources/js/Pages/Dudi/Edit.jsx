...
export default function Edit({ auth, dudi }) {
    const { data, setData, put, processing, errors } = useForm({
        nama_perusahaan: dudi.nama_perusahaan || '',
        alamat: dudi.alamat || '',
        telepon: dudi.telepon || '',
        email_perusahaan: dudi.email_perusahaan || '',
        nama_kontak_personalia: dudi.nama_kontak_personalia || '',
        latitude: dudi.latitude || '', // Field baru
        longitude: dudi.longitude || '', // Field baru
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('dudi.update', dudi.id));
    };

    return (
        <AuthenticatedLayout header="Edit Data DUDI">
            <Head title="Edit DUDI" />
                    ...
                    <form onSubmit={submit} className="p-4 sm:p-6 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <TextInput id="latitude" value={data.latitude} onChange={(e) => setData('latitude', e.target.value)} className="mt-1 block w-full" />
                                <InputError message={errors.latitude} className="mt-2" />
                            </div>
                            <div>
                                <InputLabel htmlFor="longitude" value="Longitude" />
                                <TextInput id="longitude" value={data.longitude} onChange={(e) => setData('longitude', e.target.value)} className="mt-1 block w-full" />
